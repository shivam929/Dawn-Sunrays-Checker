...
function generateProfileCard() {
  ...
  function renderCard(logoImg) {
    ...
    // Logo/Brand — draw official logo or fallback to text
    const logoSize = 28;
    const logoX = 28;
    const logoY = 26;
    if (logoImg) {
    ...
    // DAWN text - positioned after logo
    const dawnTextX = logoImg ? (logoX + logoSize + 10) : 65;
    ctx.font = 'bold 26px Syne, sans-serif';
    ctx.fillStyle = '#f97316';
    ctx.textAlign = 'left';
    ctx.fillText('DAWN', dawnTextX, 50);

    // Sunrays Tracker text - with proper spacing from DAWN
    const dawnWidth = ctx.measureText('DAWN').width;
    ctx.fillStyle = '#888';
    ctx.font = '500 13px Outfit, sans-serif';
    ctx.fillText('SUNRAYS TRACKER', dawnTextX + dawnWidth + 14, 50);
  ...
  // Try loading logo, render with or without it
  logo.onload = () => renderCard(logo);
  logo.onerror = () => renderCard(null);

  // If logo is already cached, onload may fire synchronously
...
function getShareText(user) {
  return `Checked my DAWN Sunrays standing ☀️\n\nRank: #${user.rank} globally\nSunrays: ${user.rays}\nTier: ${user.tier.name}\n\nSee where you stand:\nhttps://dawnrank.xyz\n\nBuilt by @Bunnyyxtan`;
}
